...
export default function (input: string[], { logger }: Context) {
    ...
    const grid = create(size.x + BUFFER.x, size.y + BUFFER.y, 0);
    ...
    drawLine(new Vec2(0, grid.length - 1), new Vec2(grid[0].length - 1, grid.length - 1), grid);
