...
export default function (input: string[], { logger }: Context) {
    ...
    const corners = [
        ...
        new Vec2(0, start.y),
        new Vec2(start.x, 0),
    ].map((pp) => getReachable(pp, SIZE - 1)).reduce((a, b) => a + b, 0);

    const smallCorners = [
        new Vec2(0, SIZE - 1),
        new Vec2(SIZE - 1, 0),
        new Vec2(SIZE - 1, SIZE - 1),
        new Vec2(0, 0),
    ].map((pp) => getReachable(pp, floorDiv(SIZE, 2) - 1)).reduce((a, b) => a + b, 0) * (GRID_WITH + 1);
    ...
    const largeConers = [
        ...
        new Vec2(SIZE - 1, SIZE - 1),
        new Vec2(0, 0),
    ].map((pp) => getReachable(pp, floorDiv(SIZE * 3, 2) - 1)).reduce((a, b) => a + b, 0) * GRID_WITH;
...
function getReachable(pos: Vec2, steps: number): number {
    ...
    while (queue.length > 0) {
        ...
        for (const neighbor of DIRS) {
            ...
            if (np.x < 0 || np.y < 0 || np.x >= MAP[0].length || np.y >= MAP.length || seen.has(np.toString())) {
                continue;
            }
