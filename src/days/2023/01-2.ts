...
const mapping = new Map<string, string>([['one', '1'], ['two', '2'], ['three', '3'], ['four', '4'], ['five', '5'], ['six', '6'], ['seven', '7'], ['eight', '8'], ['nine', '9']]);
...
export default function (input: string[], { logger }: Context) {
    ...
    return input.reduce((acc, line) => {
        ...
        return acc + parseInt(`${mapping.get(first) ?? first}${mapping.get(reverseString(last)) ?? last}`, 10);
    }, 0);
}
