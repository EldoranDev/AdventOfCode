...
export default function (input: string[], { logger }: Context) {
    ...
    return sum(...trailheads.map((pos) => getScore(pos, 0, map)));
}

function getScore(position: Vec2, height: number, map: Grid2D<number>): number {
    if (height === 9) {
        return 1;
    }
    ...
    for (const dir of Vec2.ULRD) {
        ...
        if (map[np.y][np.x] !== height + 1) {
            continue;
        }

        score += getScore(np, height + 1, map);
    }
